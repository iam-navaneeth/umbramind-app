...
export const WeatherHero: React.FC<WeatherHeroProps> = ({ weather }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
      ...
      <div className="glass-panel rounded-2xl p-5 sm:p-6 relative overflow-hidden flex flex-col justify-between border-cyan-500/20">
        ...
        <div>
          ...
          <div className="flex items-baseline gap-2.5 sm:gap-3 my-2">
            ...
            <span className="text-xs sm:text-sm font-semibold text-cyan-300 px-2.5 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20">
              {cond.text}
            </span>
          </div>
        </div>

        <div className="grid grid-cols-3 gap-2 sm:gap-3 mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-slate-800/80">
          <div className="flex flex-col items-center p-2 rounded-xl bg-slate-900/60 border border-slate-800">
            ...
            <span className="text-[9px] sm:text-[10px] uppercase tracking-wider text-slate-400">Humidity</span>
          ...
          <div className="flex flex-col items-center p-2 rounded-xl bg-slate-900/60 border border-slate-800">
            ...
            <span className="text-[9px] sm:text-[10px] uppercase tracking-wider text-slate-400">Wind</span>
          ...
          <div className="flex flex-col items-center p-2 rounded-xl bg-slate-900/60 border border-slate-800">
            ...
            <span className="text-[9px] sm:text-[10px] uppercase tracking-wider text-slate-400">Rain Prob</span>
      ...
      <div className="glass-panel lg:col-span-2 rounded-2xl p-5 sm:p-6 flex flex-col justify-between">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3 sm:mb-4">
          ...
          <span className="self-start sm:self-auto text-xs font-semibold px-2.5 py-1 rounded-md bg-slate-800 text-slate-300 border border-slate-700">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 overflow-x-auto pb-2 pt-1 scrollbar-thin max-w-full">
          {weather.forecast12h.hourly.map((h, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`flex-shrink-0 flex flex-col items-center p-2.5 sm:p-3 rounded-xl border w-20 sm:w-24 text-center transition-all ${
                  isHighRain
                    ? "bg-gradient-to-b from-cyan-950/60 to-blue-900/40 border-cyan-500/40 shadow-lg shadow-cyan-500/10"
                    : "bg-slate-900/50 border-slate-800"
                }`}
              >
                <span className="text-[11px] sm:text-xs font-medium text-slate-400 mb-1">{h.hourLabel}</span>
        ...
        <div className="mt-3 sm:mt-4 pt-3 border-t border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-slate-400">
          ...
          <div className="flex flex-wrap sm:flex-nowrap gap-2 sm:gap-4 overflow-x-auto pb-1 sm:pb-0">
            {weather.daily.map((d, i) => (
              <div key={i} className="flex items-center gap-1.5 px-2 py-1 rounded bg-slate-900/40 border border-slate-800/50 sm:bg-transparent sm:border-0">
                <span className="text-slate-300 font-medium">{d.dayName}:</span>
                <span className="text-slate-100 font-bold">{d.tempMax}°</span>
                <span className="text-cyan-400 text-[10px] sm:text-[11px]">({d.rainProbMax}%)</span>
              </div>
            ))}
          </div>
        </div>
